    ...
    const response = await ai.models.generateContent({
      ...
      contents: {
        parts: [
          ...
          {
            text: "Analyze this plant for any signs of disease, pests, or nutrient deficiencies. Determine its health status.",
          },
        ],
      },
      config: {
...
export const searchProducts = async (query: string): Promise<SearchResult> => {
  try {
    ...
    - "price": Price with currency symbol (e.g. "$15.99")
    - "description": 1 sentence on why it works
    - "imageUrl": A URL to the product image if you can find one in the search snippets (otherwise leave empty)
