...
import { DiagnosisResult, SearchResult } from '../types';
              ...
              {productsResult && productsResult.products && productsResult.products.length > 0 && (
                <div className="grid grid-cols-1 gap-4">
                  {productsResult.products.map((product, idx) => (
                    <div key={idx} className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm flex gap-4 hover:shadow-md transition-shadow">
                      <div className="w-20 h-20 bg-slate-100 rounded-lg flex-shrink-0 overflow-hidden flex items-center justify-center border border-slate-200">
                        {product.imageUrl ? (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start">
                          ...
                          <span className="text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded text-xs font-bold whitespace-nowrap ml-2">
                            {product.price}
                          </span>
                        </div>
                        <p className="text-xs text-slate-500 mt-1 line-clamp-2">{product.description}</p>
                        <a 
                          href={product.productUrl || (productsResult.groundingChunks?.[idx]?.web?.uri ?? "#")} 
                          target="_blank" 
